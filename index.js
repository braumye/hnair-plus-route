...
function extractTableToJson(html) {
  const $ = cheerio.load(html);
  const tables = [];
  const hasMoreTables = $('table').length > 1;
  const defaultTableName = '2025年冬春航季（2025年10月26日-2025年12月25日）';
  ...
  $('table').each((index, table) => {
    ...
    const headerRow = $(table).find('tr').eq(headerStart);
    ...
    $(table).find('tr').each((rowIndex, row) => {
      if (rowIndex <= headerStart) return; // 跳过前两行：第一行、第二行（第二行为表头）
